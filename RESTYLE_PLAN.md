...
- **`components/NutritionComponents/ScanPromptCard.tsx`** — overlay card (icon circle + title + message + gradient CTA + optional Go Back) shown over `ScanBackdrop` for upgrade / grant-permission / open-settings states. Fully tokenized (`surface`, `nutrition`, fonts, radius).
