...
- **Fatigue wheels are removed entirely** from Progress (least-accurate metric). The fatigue _algorithm_ in `WorkoutContext` stays intact (dormant), just not displayed.
...
>   tokens.ts      radius / spacing / motion                   ← static
...
- `motion` tokens (same file): `motion.imageFade=50` — the expo-image `transition` duration for exercise images (`Log`, `ScrollableList`, `addExerciseModal`, `archiveModal`). New exercise-image render sites should use it instead of a literal.
...
- **`components/NeutralComponents/DatePickerPopup.tsx`** — the app's centralized date picker (replaces the wheel-based `dateModal` route + `LogDateModal`). Composes `CenterPopup.tsx` (fade + scale-in centered dialog, tap-backdrop to dismiss) over `DateSheet.tsx` (title/subtitle + `Calendar` + gradient Confirm). `mode="workout" | "nutrition"` maps to accent/gradient/subtitle; `selectable="past"` disables future days inline (no post-hoc "invalid date" alert). Calendar internals: `Calendar.tsx` (title-zoom nav; fixed-height 6-row day grid so month paging never shifts the header) + `CalendarMonthGrid.tsx` (Monday-first grid). `BottomSheet.tsx` is the sibling slide-up + swipe-to-dismiss presentation (Dev Hub preview only). The wheel `DatePicker.tsx` is retained for the onboarding birthday picker only.
...
- **`components/NutritionComponents/FoodRow.tsx`** — shared food search-result row (name, italic brand line, muted serving-size line, optional macro-pill row, circular `+`/loading add button); extracted pixel-identical from `foodDBModal.tsx`'s inline row + staged macro pills so Popular Foods and the Dev Hub can reuse it. Fully tokenized. Unchanged visually for the universal-item-editor work, but now also renders search-preview macros (W4, food-search rows) through the same macro-pill path Popular Foods already used.
- **`components/NeutralComponents/StagedSection.tsx`** — gained an optional combine-name `TextInput` (`combineName` / `onCombineNameChange`, tokenized surface/hairline/text), shown only once "Combine items" is toggled on, placeholder `"Combined Items"`. The combine toggle row was reshaped to put the switch inline next to its "Combine items" label (one row) instead of separate. Used by the foodDB and saved-meal combine flows (universal-item-editor W2/W3); see the Dev Hub `Combine` page.
- **`components/NutritionComponents/Entry.tsx` / `SavedEntry.tsx`** — both gained an optional italic `subtitle` line under the name (12px, `textMuted`, `marginTop: 2`, `fonts.regular`, identical style in both) — the item's brand for a 1-item entry, or `"N items"` for a multi-item entry, computed by `entrySubtitle()`. See the Dev Hub `Entry Rows` page.
- **Dev Hub lab retired (universal-item-editor, 2026-07-16):** the old editPhoto variants lab (`EditPhotoEntryTest`, `EditPhotoModalHost`, `EditPhotoVariants` + its 2 route stubs) is deleted, replaced by the `Edit Entry` test page (`components/devTest/EditEntryTest.tsx`, `/devTest/editEntry`) covering the one unified editor for every entry type.
...
`app/settingsScreens/subscription.tsx` mirrors the V4 onboarding paywall (`app/onboardingScreens/paywall.tsx`): minimal header, blue funnel (blue `workoutGradient` CTA + blue selected price card + blue "Best value"), neutral feature chips, ★★★★★ 5.0 + trust lines, prices from `useBilling` (`priceInfo`/`annualPriceInfo`). It is a purchase CTA (onboarding-conversion analog), so it keeps the blue gradient — the neutral-CTA rule above is for _commit/save_ buttons, not the subscribe button. Settings-specific bits kept: `beforeRemove` purchase guard, Manage-subscription link, success Alerts, and the `hasPremium` "Subscription Active" state (no onboarding plan card / footer). The two paywalls are intentionally kept as separate files.
...
- **Manual entry modals (`addNutritionModal.tsx`, `editManualEntry.tsx`) — migrated to the AI-first design** (shipped from Dev Hub variant 4b): left-aligned hero title + sub, multiline describe field, gradient `Generate macros` primary (premium-gated; locked state routes to subscription), `or enter manually` divider, macros grouped in a borderless `surfaceInset` panel (calorie hero row + Protein/Carbs/Fats cells), pinned safe-area footer CTA disabled until content. Edit screen = same panel without the AI section.
...
- **Surface tiers by context:** elements sitting on the screen `background` use `surface` (raised); use `surfaceInset` for things nested _inside_ a `surface` card **or** for quiet grouping panels directly on the background. (`surfaceInset` is a true middle tier in both palettes — light `#E9EBF0` sits between the `#DFE2E8` background and `#FBFBFC` cards, mirroring dark.)
- **Spacing = grouping (proximity), not uniformity.** Use a few steps from an 8/4 scale, chosen by relationship — never space everything equally:
    - _Within a cluster/pair_ (icon→title, title→subtitle, a pair of links, items in a row): **2–8**.
    - _Within a group_ (section title→card, row padding): **12**.
    - _Between groups/sections_: **24** on scrollable list screens (profile, settings), **16** on compact no-scroll screens (subscription).
    - _Distinct tier_ (legal/fine print, destructive actions): separate further (**~24**) or anchor to the bottom — don't let it share a group's gap.
...
>
> **Progress (branch `restyle/primitives-rollout`).** Shared primitives that render _inside_ already-migrated screens are now fully token-driven: `ScrollableList`, `DatePicker`, `CustomHeader` (NeutralComponents), `SelectionModal` (GraphComponents), and `AppLoadingScreen` (GuardComponents) — including the last lime `#22C922` accent bar/thumbnail → `colors.nutrition` (emerald). `authScreens/login.tsx` migrated (terms link → `workoutInk`). **Layout retune (follow-up pass):** those primitives' card corners were then tightened from the old 16px literals to the `radius.*` tokens (`radius.card` 10 for list/picker cards, `radius.cardLg` 12 for the DatePicker header + login Apple button), `SelectionModal` inset 25→24, and `ScrollableList` cards adopted the reference **flat-in-dark / soft-in-light** shadow (matching `Entry`/`Log`) with `hitSlop` added to the `✕`/back-button glyphs for ≥44pt targets. Padding/gaps were left as literals — the Refined system tokenizes corners, not spacing. Residual-hex sweep of the already-migrated screens is complete: the remaining literals are **documented intentional exceptions** — white-on-accent/gradient text, black shadows, `cameraScreen` overlay chrome (and its `ScanBackdrop` mirror — must stay in sync with `cameraScreen.tsx`), `ProgressWheel` ring gradient stops, the `(tabs)/_layout` fixed dark "ink" tab bar, the web-only phone-chrome frame in `app/_layout.tsx` (sits outside `ThemeProvider`), and the Skia `useFont` loaders in `Graph1`/`BarChart` (already `FONT_FAMILY`-switched). **Still deferred:** `app/onboardingScreens/` (11 files) — excluded pending the `components/devTest/onboarding/` V4 redesign that will replace it; its onboarding-specific token gaps (macro colors `#FF6B6B`/`#FFD93D`, light-tinted CTA backgrounds, gradient-overlay rgba) are deferred with it.
>
> **Loading screen redesign (2026-07).** `AppLoadingScreen` replaced the rotating-logo-in-a-circle with an **orbit loader**: static bare logo (no circle backing) with a strong breathe pulse (scale 1→1.12, 1.1 s each way, starts after the entrance), two brand dots orbiting it — 8 px `colors.workout` on a fast 144 px outer orbit (1.3 s/rev), 6 px `colors.nutrition` on a slower 116 px inner orbit (2.1 s/rev) — and a staggered entrance (logo fades/scales in, rings follow 250 ms later). The caption is now **delayed tracked caps**: hidden for the first 1 s (fast loads show no text), then fades up in `textSecondary` / `fonts.semibold` / 12.5 px / +1.6 tracking / uppercase, with trailing ellipses stripped (the motion is the progress signal). Same `message` prop API. All candidate variants remain viewable in the Spinner Lab dev page (`/devTest/spinnerLab`, `components/devTest/SpinnerVariants.tsx`).
