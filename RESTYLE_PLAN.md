...
> **Onboarding is now restyle-ready (view/controller split).** All 11 onboarding screens were refactored: each `app/onboardingScreens/*.tsx` is now a thin **controller** (keeps `useSettings`/`router`/`useBilling`/validation/macro-calc/`completeOnboarding` — behavior unchanged) that renders a pure presentational **view** in `components/OnboardingComponents/*View.tsx`. The views still hardcode the old dark hexes — restyle them there (single source of truth). A `__DEV__`-only **Onboarding preview** (Dev Hub → Flows → Onboarding) steps through all screens with mock data + a per-step **variant** picker + Light/Dark toggle (`components/devTest/OnboardingFlowTest.tsx` + `onboarding/registry.tsx`), so you can A/B redesigns without logging out, then graduate a variant into the shared view. No real writes / RevenueCat / `onboardingComplete` flip in the preview.
