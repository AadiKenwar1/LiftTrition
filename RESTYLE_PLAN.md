...
- **Contrast tweaks (a11y):** dark gradients deepened so white CTA text clears contrast (the bright accents gave low white-on-color ratios — green ~2:1, blue ~3.4–3.9:1): `nutritionGradient` → `['#1FA51F','#178A17']`, `workoutGradient` → `['#2E7BE0','#2570D8']`. The `nutrition`/`workout` accents themselves are unchanged (icons/borders stay vivid). Muted text nudged to AA 4.5:1: `labelMuted` light `#6C6D75`→`#5A5B64`, dark `#6B6B73`→`#82838C`; `textMuted` light `#6A6A6E`→`#5E5E62`.
