...
- **Surface tiers by context:** elements sitting on the screen `background` use `surface` (raised); only use `surfaceInset` for things nested _inside_ a `surface` card. (`surfaceInset` == `background` in light mode, so an on-background inset disappears.)
