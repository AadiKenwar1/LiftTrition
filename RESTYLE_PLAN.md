...
- **Contrast tweaks (a11y):** gradient *fills* deepened to full AA so white CTA/icon text clears 4.5:1 in **both** modes (the bright accents gave low white-on-color ratios — green ~2:1, blue ~3.4:1): `nutritionGradient` → `['#168516','#0F7A0F']` (white ~4.8–5.5:1), `workoutGradient` → `['#2570D8','#2064C8']` (white ~4.8–5.6:1). Same deep pair in light + dark — the fill is self-contained. These tokens also drive the **Fab** and **ModeSwitcher** active segment (white icons), so those read deeper/less-neon but pass the 3:1 graphic bar. The `nutrition`/`workout` **accents** are unchanged (icons/borders/rings/numerals stay vivid — the dominant ~189-usage role). Muted text nudged to AA 4.5:1: `labelMuted` light `#6C6D75`→`#5A5B64`, dark `#6B6B73`→`#82838C`; `textMuted` light `#6A6A6E`→`#5E5E62`. Known minor: `StagedSection` count badge draws white on the raw accent (not the gradient), still ~2:1 on dark for the tiny number.
