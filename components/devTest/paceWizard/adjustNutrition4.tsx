...
// Dev duplicate of app/settingsScreens/adjustNutrition/adjustNutrition4.tsx — see the step-1 copy's docblock
// for the why and the sync rule. The pace-derivation prototype landed in the original (projectGoal derives
// the quoted weeks from the calorie target actually leaving step 3, wrong-direction renders "—", maintain
// keeps the 12-week horizon), so this copy now matches it line for line; the only remaining deltas are this
// docblock and the dismissTo comment naming the dev launcher.
...
export default function AdjustNutrition4Screen() {
    ...
    const merged = useMemo(
        () => ({
            ...
            goalType: variant,
            goalWeight,
            goalPace: Number(params.goalPace) || 0,
        }),
    ...
    const handleSave = () => {
        ...
        // Return to the Settings tab AND drop the whole finished wizard (and the dev launcher under it) from
        // history — pushing would stack Settings on top, letting a hardware/swipe back re-enter a stale flow.
        // dismissTo pops back to the existing Settings screen, same as the original.
