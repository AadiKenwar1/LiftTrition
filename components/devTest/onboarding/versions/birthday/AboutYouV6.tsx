...
/**
 * Dev-only V6 About You — V5's screen with only the copy changed. The title carried the most weight here: "About
 * you" is generic, and a generic heading over sex, date of birth and height leaves the subtitle to do all the
 * work of explaining why three personal questions are being asked. Naming the payoff in the title does it first
 * instead. The subtitle was one long sentence hinged on an em dash, the same shape four other subtitles in the
 * flow used; it is now two plain sentences, and its "all three" tells the reader the screen is short. "Calorie
 * goals" also becomes "targets", the word the activity and plan screens already use for the same numbers, so
 * the flow stops cycling synonyms for one thing.
 *
 * Everything else matches V5: sex + DOB + height only, the DOB guardrail (the picker must be touched before
 * Next enables, so no invented age reaches the BMR term), and the 13+ check. Height is validated on its own
 * with validateHeight, so no stand-in weight is needed. Inert.
 */
export default function AboutYouV6() {
    ...
    function handleBeforeNext(): boolean {
        ...
        if (!validateHeight(totalHeight, unitSystem)) return false
        const now = new Date()
    ...
    return (
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
            <View style={{ flex: 1 }}>
                <V4Screen step={4} totalSteps={8} eyebrow="Step 5 of 8" title="Your calorie baseline" subtitle="Age, height and biological sex all change how many calories you burn. We need all three to set your targets." accent={accent} nextDisabled={!filled} onBeforeNext={handleBeforeNext} onBack={() => router.back()} onNext={() => {}}>
