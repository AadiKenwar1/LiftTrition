...
/** Dev-only V3 (black & white) Macros / personalized plan — green accent, editable cells. Inert. */
export default function MacrosV3() {
    ...
    return (
        <View style={{ flex: 1 }}>
            <V3Screen step={9} eyebrow="Step 10 of 12" title="Your plan is ready" subtitle="Here are your daily targets — tap any to fine-tune it." accent={accent} onBack={() => router.back()} onNext={() => {}}>
                    ...
                    {CARDS.map(({ kind, Icon, label, value }, i) => (
                        <Animated.View key={kind} entering={FadeInDown.delay(i * 50).duration(280)} style={styles.cell}>
                            <PressableScale style={styles.card} onPress={() => setEditingKind(kind)}>
                                ...
                                <Icon size={18} color={colors.nutrition} strokeWidth={2.2} />
