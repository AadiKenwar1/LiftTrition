...
import { ActivityIndicator, Alert, Keyboard, Modal, ScrollView, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native'
...
// The new Food DB experience, presented as a pristine modal (no dev chrome) —
// the simulated tier comes in via the ?tier= route param from the FoodDBTest
// launcher. Task 6 ports this composition into the real foodDBModal.
export default function FoodDBModalPreview() {
    const { handleAddNutrition, selectedDate } = useNutrition()
    const { userID } = useAuth()
    const router = useRouter()
    const params = useLocalSearchParams<{ tier?: string }>()
    const simFree = params.tier !== 'premium'
    ...
    useEffect(() => {
        ...
        const timeoutId = setTimeout(async () => {
            ...
            try {
                ...
                setSearchResults([])
                // Kept in sync with the real foodDBModal (audits C1 + H1): surface the daily-quota
                // and premium-gate messages verbatim, generic connection copy otherwise.
                const message =
    ...
    function handleAddAll() {
        ...
        setAddedItems([])
        Alert.alert('Added', `Logged ${count} item${count > 1 ? 's' : ''} to today's nutrition.`)
    }
