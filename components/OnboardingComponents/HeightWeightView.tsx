...
export interface HeightWeightPayload {
    height: number
    bodyWeight: number
...
export interface HeightWeightViewProps {
    /** Emits the computed height (total inches / cm), weight (in `unitSystem`), and unit system. */
    onNext: (payload: HeightWeightPayload) => void
    onBack: () => void
    stepIndex: number
}
...
export default function HeightWeightView({ onNext, onBack, stepIndex }: HeightWeightViewProps) {
    ...
    function handleNext() {
        ...
        onNext({ height: totalHeight, bodyWeight: Number(weight), unitSystem })
    }
